...
function download(URL, downPath)
{
    ...
    if (DownMode == '') {
        ...
        try{
          XMLHTTP.setRequestHeader('Referer', 'http://tools.js.bathome.net/');
        }
        catch(Err){}
        XMLHTTP.Send();
        ADO.Mode = 3;
...
{
    //修改时间大于1分钟的列表才需更新
    if(FSO.FileExists(bcnPath + 'tool.@version.txt')){
      var listLastModified = new Date(FSO.GetFile(bcnPath + 'tool.@version.txt').DateLastModified)
    } else {
      return false
    }
    if(Math.abs(listLastModified - (new Date()).getTime()) <= 60000) return true
